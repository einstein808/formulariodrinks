...
export function calculatePackagePrice(pacote, guestCount, duration, options = {}) {
  ...
  const pricingMode = isMaoDeObra 
    ? 'tier' 
    : (isGroupB ? 'tier' : 'person');
  ...
  const isTier = hasTiers && (pricingMode === 'tier');
  
  // Detectar se é por pessoa (Grupo A, exceto mão de obra)
  const label = (pacote?.priceLabel || '').toLowerCase();
  const isPerPerson = !isTier && !isMaoDeObra && (pricingMode === 'person');
