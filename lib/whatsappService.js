...
export const sendWhatsAppQuote = async (formData, pacotes, leadId = null) => {
  ...
  try {
    ...
    console.error('Erro ao obter configuração geral no Firebase:', error);
  }
  ...
  // Usar apenas pacotes visíveis (não ocultos da vitrine)
  const pacotesValidos = pacotes.filter(p => !p.hidden);

  pacotesValidos.forEach(p => {
    // Determine if package is per person
    const isPerPerson = p.priceLabel && p.priceLabel.toLowerCase().includes('por pessoa');
