...
export function cleanPhoneForWhatsApp(phone) {
  ...
  if (number.startsWith('55')) {
    const dddAndNum = number.substring(2);
    if (dddAndNum.length === 10) {
      const ddd = dddAndNum.substring(0, 2);
      const num = dddAndNum.substring(2);
  ...
  return number.length >= 10 ? '55' + number : number;
}
